...
async function mintTunnelToken(secret: string, sessionId: string): Promise<string | null> {
...
export default tool({
  ...
  async execute(args) {
    try {
      ...
      const shellFormat = filePath.endsWith(".sh")
      const lines: string[] = []
      const pushEnv = (key: string, value: string) => {
        lines.push(shellFormat ? `export ${key}=${shellQuote(value)}` : `${key}=${value}`)
      }

      if (args.include_gateway !== false && status?.tunnelUrls?.gateway) {
        pushEnv("GATEWAY_URL", status.tunnelUrls.gateway)
      }

      if (tokenValue) {
        pushEnv("TUNNEL_TOKEN", tokenValue)
      }
      ...
      for (const tunnel of tunnels) {
        ...
        if (!url) continue
        pushEnv(toEnvKey(prefix, tunnel.name), withToken(url))
      }
      ...
      if (apiUrl) pushEnv("API_URL", withToken(apiUrl))
      if (frontendUrl) pushEnv("FRONTEND_URL", withToken(frontendUrl))
      if (backendUrl) pushEnv("BACKEND_URL", withToken(backendUrl))

      const header = [
        "# Auto-generated by write_tunnel_env",
        `# Session: ${sessionId}`,
        tokenValue ? "# Tunnel auth token included" : "# Warning: no tunnel auth token available",
        "",
      ]
